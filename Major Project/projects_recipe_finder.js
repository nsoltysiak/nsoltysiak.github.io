...
const favoriteRecipesHeader = document.querySelector(
  ".favorite-recipes-header"
);
...
function addToFavorites(meal, countUpdate = true) {
  ...
  const healthScore =
    meal.healthScore ||
    meal.nutrition?.nutrients?.find((n) => n.name === "Health Score")?.amount ||
    meal.healthScore ||
    0;
...
async function getRecipes({ type, value }) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const results =
        type === "ingredient" ? jsonResponse : jsonResponse.results;

      if (type === "ingredient") {
        const detailed = await Promise.all(
          results.map((m) =>
            fetch(
              `https://api.spoonacular.com/recipes/${m.id}/information?includeNutrition=true&apiKey=${userApiKey}`
            ).then((r) => r.json())
          )
        );
    ...
    console.log(error);
    alert(
      "Failed to fetch recipes. Please check API key or try again tomorrow."
    );
  }
}
...
async function openRecipeModal(meal) {
  ...
  if (!meal.extendedIngredients) {
    try {
      const response = await fetch(
        `https://api.spoonacular.com/recipes/${meal.id}/information?includeNutrition=true&apiKey=${userApiKey}`
      );
  ...
  const isAlreadyFavorite =
    document.getElementById(`favorite-${meal.id}`) !== null;
